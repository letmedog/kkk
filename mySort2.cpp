...
int main(int argc, const char * argv[]) {
    string pattern = ";";           //";" as field separator
    ...
    if(in){
        while(getline(in,line)){
            ...
            if (splitCheck == 2) {
                ...
                // It should be a positive integer, but less than 200
                int checkAge = 0;
                ...
                // Check if the code field is a four digit number with leading zero(es)
                int checkCode = 0;
                if (result[0].length() == 4) {
                    for (int codeNum = 0; codeNum < result[0].length(); codeNum++){
                        if (result[0][0] != '0') {
                            break;
                        }
                        if (isdigit(result[0][codeNum])) {
                            checkCode++;
                        }
                    }
                ...
                if (checkFormat == 0 && checkSame == 0 && checkInt > 0 && checkAge > 0 && checkCode == 4) {
                    for (int i = 0; i<3; i++) {
                        data[n][i] = result[i];
                    }
